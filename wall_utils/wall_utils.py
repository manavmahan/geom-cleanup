...
def collect_all_lines(perimeter, spaces):
    lines = []
    for i in range(len(perimeter.exterior.coords) - 1):
        lines.append(RLineString([perimeter.exterior.coords[i], perimeter.exterior.coords[i + 1]]))
    for s in spaces:
        space = spaces[s].exterior.coords
            ...
            for j in range(i + 1, len(lines)):
                ...
                if x is not None and x.length > overlap_threshold:
                    ...
                    if y is not None:
                        ...
                        if (not any(
                            [temp_spaces[s].contains(mid_point) for s in temp_spaces]
...
def get_wall_triangles(example, min_area=2):
    ...
    all_lines = collect_all_lines(perimeter_polygon, spaces)
